  ...
  void _login() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (authNotifier.state.status == AuthStatus.authenticated) {
        ...
        await prefs.setString('currentUserId', authNotifier.state.user!.uid); // Save the actual UID
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(authNotifier.state.errorMessage ?? 'Login Failed')),
        );
      }
    }
  }
                ...
                Consumer<AuthNotifier>(
                  builder: (context, authNotifier, child) {
                    return ElevatedButton(
                      onPressed: authNotifier.state.status == AuthStatus.loading ? null : _login,
