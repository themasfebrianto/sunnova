...
Future<void> seedDatabase(DatabaseHelper databaseHelper) async {
  // 1. Delete database file completely (fresh seed)
  await databaseHelper.deleteAllDatabases();

  // 2. Reinitialize DB (creates new, empty schema)
  final db = await databaseHelper.database;

  // 3. Seed inside a single transaction (fast & atomic)
  await db.transaction((txn) async {
    ...
    for (final badge in badges) {
      await txn.insert('badges', badge.toMap());
    }
    ...
    await txn.insert('course_modules', module1.toMap());
    ...
    await txn.insert('course_modules', module2.toMap());
    ...
    await txn.insert('lesson_units', lesson1_1.toMap());
    ...
    final lesson1_2 = LessonUnitModel(
      ...
      description:
          'Memahami hukum nun mati dan tanwin (idzhar, ikhfa, iqlab, idgham)',
    ...
    await txn.insert('lesson_units', lesson1_2.toMap());
    ...
    for (final q in questions) {
      await txn.insert('assessment_questions', q.toMap());
    }
    ...
    for (final a in userAchievements) {
      await txn.insert('user_achievements', a.toMap());
    }
